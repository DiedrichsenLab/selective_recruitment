...
#  
def prep_sc_whole(outpath = prep.conn_dir+'/Demand'):
    """
    ...
    filepath = os.path.join(outpath, 'sc_df_whole_ses-01.tsv')
    df.to_csv(filepath, index = False, sep='\t')
    return

# 
def prep_sc_roi(outpath = prep.conn_dir+ '/Demand'):
    """
    ...
    # save the dataframe for later
    filepath = os.path.join(outpath, 'sc_df_VWM_ses-01.tsv')
    df.to_csv(filepath, index = False, sep='\t')
    return

# 
def prep_sc_conn(outpath = prep.conn_dir+'/Demand'):
    """
    Getting the summary dataframe for the scatterplot in an ROI-wise manner
    """
    ...
    # save the dataframe for later
    filepath = os.path.join(outpath, 'sc_df_VWM_conn_ses-01.tsv')
    df.to_csv(filepath, index = False, sep='\t')
    return
