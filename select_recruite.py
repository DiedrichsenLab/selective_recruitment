...
# 6. getting data into a dataframe
def get_summary_roi(dataset_name = "WMFS", info_subset = None):
    """
    ...
    label_cereb = atlas_dir + '/tpl-SUIT' + '/atl-Buckner7_space-SUIT_dseg.nii'

    label_cortex = []
    for hemi in ['L', 'R']:
        label_cortex.append(atlas_dir + '/tpl-fs32k' + f'/yeo7.{hemi}.label.gii')
    ...
    for sub in range(len(T.participant_id)):
        ...
        # getting parcel axis 
        # pax = cifti_X.header.get_axis(1)
        pay = cifti_Y.header.get_axis(1)
        ...
        for ilabel in range(len(pay.name)):
            info_sub = info.copy()
...
if __name__ == "__main__":
    ...
    """
    df = get_summary_roi(dataset_name = "WMFS")
    # save the dataframe for later
    filepath = os.path.join(base_dir, 'WMFS', 'sc_df_buckner7_ses-02.tsv')
    df.to_csv(filepath, index = False, sep='\t')
